-- Seed dev sales data into hourly_sales_data for a given workplace.
-- This is the SAME data the ML engine reads from its bundled XLS/PDF files,
-- averaged by day-of-week and mapped onto each calendar week.
--
-- Usage: Run in Supabase SQL Editor.
--   1. For Mary Browns: just run as-is (auto-finds by name)
--   2. For a test account: change the WHERE clause at the bottom
--
-- The script seeds 8 weeks of sales data (4 past + 4 future) so the
-- API fallback (loadSalesForWorkplace) always has data regardless of
-- which week the schedule targets.
...
DECLARE
  ...
  v_monday DATE;
  v_day DATE;
  v_dow INT; -- 0=Mon in Python convention
  week_offset INT;
BEGIN
  -- Find workplace
  ...
  -- Dev sales averages by (python_weekday 0=Mon, hour)
  ...
  -- Seed 8 weeks centered around today
  FOR week_offset IN -4..3 LOOP
    ...
    FOR v_dow IN 0..6 LOOP
      ...
      SELECT
        v_wp_id, v_day, s.hour, s.sales,
        -- JS getUTCDay convention: 0=Sun, 1=Mon ... 6=Sat
        CASE v_dow WHEN 6 THEN 0 ELSE v_dow + 1 END,
        'seed'
      FROM _dev_sales s
      WHERE s.dow = v_dow
  ...
  RAISE NOTICE 'Sales data seeded for workplace % (8 weeks)', v_wp_id;
END $$;
